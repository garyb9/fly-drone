...
world.scene.fog = new THREE.Fog(color.bg, 7, 20);
const grid = new THREE.GridHelper(12, 48, 0x435a63, 0x243640);
world.scene.add(grid);
const floor = new THREE.Mesh(
  new THREE.PlaneGeometry(18, 18),
  new THREE.MeshStandardMaterial({ color: 0x14232b, roughness: 0.95 }),
);
...
for (const [i, [x, y]] of motorXY.entries()) {
  ...
  rotor.add(box(0.045, 0.004, 0.0015, mat(i % 2 === 0 ? color.cyan : color.orange)));
  rotors.push(rotor);
  body.add(rotor);
  const ring = new THREE.Mesh(
    new THREE.RingGeometry(0.022, 0.023, 40),
    new THREE.MeshBasicMaterial({
      color: i % 2 === 0 ? color.cyan : color.orange,
...
const target = new THREE.Mesh(
  ...
  new THREE.MeshStandardMaterial({ color: 0xffe795, emissive: 0xa16b15, emissiveIntensity: 0.4 }),
);
world.scene.add(target);
const obstacle = new THREE.Mesh(new THREE.SphereGeometry(0.25, 24, 16), mat(0x596c77));
...
new GLTFLoader().load(
  ...
  undefined,
  () => {
    fly.add(new THREE.Mesh(new THREE.SphereGeometry(0.15, 16, 16), mat(color.cyan)));
  },
);
...
function update(f: Frame) {
  ...
  if (points && metadata) {
    ...
    const base = new THREE.Color(0x3b7483),
      hot = new THREE.Color(color.cyan);
