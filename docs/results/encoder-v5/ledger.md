...
- Phase 1b (entropy regime): `build_sac` now sets `ent_coef="auto_0.01"` and `target_entropy = dim·(0.5·ln(2πe) + WARM_START_LOG_STD)` ≈ −1.081·dim (was SB3 default alpha init 1.0, target −dim). New `ClampedActor` clamps log_std to [−4, −1] (SB3's own cap is [−20, +2], sigma up to 7.4); `WARM_START_LOG_STD = −2.5` replaces the literals at both warm-start sites. 3 tests (fresh α=0.01 and target entropy for 4/8 dims; clamp holds at ±extremes; resume carries saved α). Full suite 156 passed, ruff clean. Design spec §8.6 records the amendment.
- Phase 1c (logging): new `ProbeLogger` (every 1000 steps) records on a fixed seeded probe batch `probe/log_std_mean`, `probe/action_mean`, `probe/action_state_std` (variation of the deterministic action across states); wired beside `MetabolicLogger` in `train_round`. The third is the round-1 collapse signature that `log_std` alone misses. 1 test: log_std recorded, and zeroing the actor's `mu` drives `action_state_std` → 0. Full suite 157 passed, ruff clean.
- Phase 1d (Pack 1): R1 fixed-probe E1/E2 — `_checks_job` gains `controller` ("teacher" flies the geometry-driven teacher so trajectories/labels are encoder-independent; "policy" is the old confounded mode), `encoder_checks` defaults to `controller="teacher"`, `validate` reports fixed-probe E1/E2 and keeps `E1_policy`/`E2_policy`, CLI `encoder-checks --controller`. R2 per-axis clone error (`{split}_{vx,vy,vz,yaw}_mse`) in `warm_start_decoder`. R3 `peak_vy`/`peak_vz` on each threat outcome. R4 `ActionLogger` records `rollout/action_vy_absmean`/`action_vz_absmean` (decoder only). R5 is covered by the Phase 1a guard. R6 worker defaults 16 → 6 in `roam_eval` (`evaluate_free_roam`, `skill_probes`) and `distill` (`collect`, `screen`). 5 tests. Full suite 162 passed, ruff clean.
- Session close 2026-09-16: Phase 0 + Phase 1a–d landed and pushed (6cb5173, b471d06, 0f5deee, a7d9475); nothing running. Phase 1e (Pack 2 S1–S4) not started. Outstanding before the re-run: Pack 2, the v4 fixed-probe E1 reference, Phase 1.5 pipeline re-plan (shorter rounds + mid-round checkpoint validation), Phase 2 re-run (needs user approval). Status doc: docs/results/encoder-v5/RECOVERY-STATUS-2026-09-16.md.
- Phase 1e (Pack 2): S4 critic-only warm-up loop skips the actor forward/backward entirely (`WarmupSAC._train_critic_only`, mirrors SB3 2.9.0 `SAC.train` with the actor and entropy losses removed); the actor-loss forward pass is gone, not just its optimiser step. S3 n-step Bellman targets (opt-in `--n-step`, default 1) and S1 memory-optimised buffer (obs stored once, next_obs derived; `--optimize-memory`) share the new `replay.FlyDictReplayBuffer`; n-step stops at done or timeout. S2 crash-resume: `ResumeCheckpoint` keeps one matched model+buffer snapshot (`resume/model.zip`, `resume/buffer.pkl`) every `resume_every` model steps; `--resume` continues it (restores `num_timesteps`, so the warm-up is not repeated) and runs the remainder of the round's `frames`; the snapshot is removed when the round completes. Defaults (n_step=1, optimize off, resume off) reproduce the pre-Pack-2 path exactly, pinned by `test_the_default_mode_reproduces_a_plain_dict_buffer`. 8 tests. Full suite 170 passed, ruff clean.
- v4 fixed-probe E1 reference (runs/v5/e1-v4-fixed.json, teacher controller, 50 seeds): E1 loom AUC **0.732** (fail ≥0.8; +0.046 over the 0.687 policy-flown baseline), E2 pass (light margin 0.444, loom margin 0.254). The fixed probe raises the number but confirms the E1 bar is not met even by the healthiest encoder; it is reported alongside, never used to move the bar. Both records archived under docs/results/encoder-v5/run-records/.
