...
mid_gate() {
    local enc="$1"
    log "mid-round liveness check on $enc"
    $PY scripts/encoder_liveness.py "$enc" 2>&1 | while read -r line; do log "  $line"; done
    $PY scripts/encoder_liveness.py "$enc" >/dev/null 2>&1
}
...
log "T11 round 1 mid-round liveness gate"
if ! mid_gate runs/v6/round1/encoder/encoder.pt; then
    rm -rf runs/v6/round1/encoder/resume
    log "T11 STOP: encoder is degenerate by $ENC_MID_FRAMES; keeping round 0"; exit 2
fi
