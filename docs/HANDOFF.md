...
**Written 2026-09-20 after P4 closed and C4a A0–A2 landed. Last commit `786c1e7` (branch `main`,
pushed).** This is the operational companion to the governing spec; read it before touching code.
...
- **C4a is half-done.** The connectome *senses* loom, but the bridge climbed instead of steering.
  Codec **v3** now strafes away from the looming side and passes its **free falsifier**; the
  **15-seed A3 gate has not been run**, and **C4b (selective loom / wall avoidance) has not started**.
- **The shipped default is now codec v3** (`DEFAULT_CODEC = "v3"`), by user direction, **ahead of its
  A3 gate**. v2 remains a selectable fixed codec and the P4 default; the gate is still required
  before any *capability* claim (and before the 50-seed stage).
- **Nothing is blocked.** The next action is one command (§5). It is a ~1 h run and needs the user's
  go-ahead (AGENTS: ask before long runs).
...
| **C4** (now) | Use the brain's escape response + selective loom | **C4a A0–A2 landed; A3 pending; C4b not started** |
...
- **A3 (NOT run):** the 15-seed gate — see §5.
- **C4b (NOT started):** a selective loom front-end (the documented v6 spatial route, or the P3
  relay extended to LC4/LPLC2), gated by the E1/E2 selectivity tests, needed for **wall avoidance**
  and for A4 to be causal. A head-on wall is a symmetric loom, so C4a alone will not fix A2.

## 5. Exact next action

Run the pre-registered A3 gate for v3 and compare to the v2 smoke (same seeds):

```bash
env -u PYTHONPATH .venv/bin/fly-drone adapter-check --adapter v3 \
    --episodes 15 --seconds 120 --level 3 --workers 6 --seed-base 1000 \
    --output docs/results/adapter/adapter-check-v3-smoke.json
```

Read: near-threat peak `|vy|`, **A3 dodge/balanced/ghost** (ghost must be ≤0.3), A4 loom ratio, A2
collisions, A6 (no `slow_fraction` regression). Also re-check liveness has not regressed:

```bash
env -u PYTHONPATH .venv/bin/fly-drone liveness-check --adapter v3 \
    --episodes 15 --seconds 120 --level 3 --workers 6 --seed-base 2000 \
    --output docs/results/liveness/liveness-check-v3-smoke.json
```

Then decide: v3 is already the default (user override), but the A3 gate still decides whether the
lateral-escape change is real and whether to spend the 50-seed stage. **Ask the user before the
50-seed stage-3 gate and before starting C4b.**
...
- **Default bridge is codec v3** (`adapter.DEFAULT_CODEC = "v3"`, C4a; made default by user
  direction ahead of its A3 gate); v2/v1 are opt-in. The viewer cycle is v3/v2/v1.
...
`…-loom-escape-audit.md`, `…-c4a-a0.md`, `…-c4a-a1-a2.md`.
...
`adapter-v2.json` (v2), `adapter-v3.json` (v3, **shipped default**), plus `adapter-check*.json`,
`adapter-check-v2-smoke.json`, `escape-side-probe.json`, `escape-command-audit.json`.
...
- **v3 is the default ahead of its gate** (user override). The A3 + 50-seed gates still decide
  whether the lateral-escape change is real and whether it becomes the *capable* default.
- **C4b is required** for A2/A4 (wall avoidance); it is a front-end change with the documented
  E1/E2 selectivity gates and needs sign-off.
